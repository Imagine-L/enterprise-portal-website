<template>
  <div class="page-header">
    <el-page-header @back="goBack" title="返回">
      <template #content>
        <span class="text-large font mr-3"> 新建文章 </span>
      </template>
      <template #extra>
  ...
  <div class="editor-box">
    <div style="border: 1px solid #ccc; margin-top: 10px;">
      ...
      <Editor
          ...
          v-model="saveArticleQuery.htmlContent"
          @onCreated="onCreated"
          :align="'left'"
      />
    </div>
  ...
  >
    <el-form :model="saveArticleQuery" ref="articleRef" label-width="80px" label-position="left">
      <el-form-item label="文章标题" prop="title" :rules="checkRules.title">
        <el-input v-model="saveArticleQuery.title" placeholder="请输入文章标题"/>
      </el-form-item>
      <el-form-item label="绑定栏目" prop="bindNavArr" :rules="checkRules.bindNavArr">
        <el-cascader :options="multiNavList" clearable placeholder="请选择绑定栏目" v-model="saveArticleQuery.bindNavArr"/>
      </el-form-item>
      <el-form-item label="文章摘要" prop="summary" :rules="checkRules.summary">
        <el-input v-model="saveArticleQuery.summary" type="textarea" placeholder="请输入文章摘要"/>
      </el-form-item>
      <el-form-item label="禁用">
        <el-switch v-model="saveArticleQuery.disabled"/>
      </el-form-item>
      <el-form-item label="发布">
        <el-switch v-model="saveArticleQuery.released"/>
      </el-form-item>
      <el-form-item label="排序种子">
        <el-input-number v-model="saveArticleQuery.orderSeed"/>
      </el-form-item>
      <el-form-item>
  ...
  >
    <div class="preview-box">
      <div class="preview-title">
        {{ saveArticleQuery.title !== null && saveArticleQuery.title !== '' ? saveArticleQuery.title : '示例标题' }}
      </div>
      <el-divider class="preview-divider"/>
      <p class="preview-description" v-html="previewDescription"></p>
      <div class="preview-content" v-html="saveArticleQuery.htmlContent"></div>
    </div>
    <template #footer>
...
export default {
  ...
  methods: {
    ...
    handleSave() {
      ...
      this.saveArticleQuery.summary = text.substring(0, 50)
      // 获取多级列表
      this.getMultiNav()
    ...
    doSave() {
      this.$refs['articleRef'].validate((valid) => {
        if (valid) {
          this.saveArticleQuery.navId = this.saveArticleQuery.bindNavArr[this.saveArticleQuery.bindNavArr.length - 1]
          saveArticle(this.saveArticleQuery).then(resp => {
            if (resp.success) {
